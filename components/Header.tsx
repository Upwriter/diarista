      ...
      <div className="mx-auto flex max-w-content items-center justify-between px-5 py-4">
        <Link href="/" className="flex items-center gap-2" aria-label={`${SITE.nome} — página inicial`}>
          <span aria-hidden className="grid h-9 w-9 place-items-center rounded-xl bg-brand text-paper">
            {/* pin de localização = "perto de mim" */}
        ...
        <nav className="flex items-center gap-6 text-sm font-medium">
          <Link href="/#como-funciona" className="hidden text-ink/70 transition-colors hover:text-brand sm:block">
            Como funciona
          </Link>
          <Link
            href="/diarista/pinheiros"
            className="hidden text-ink/70 transition-colors hover:text-brand sm:block"
          >
            Bairros
          </Link>
          <Link
